...
import '../../shared/pdu_vcu_status_ch1_4/di/pdu_ch1_4_providers.dart';
import '../../shared/pdu_vcu_status_ch5_8/di/pdu_ch5_8_providers.dart';
...
class PowerStateNotifier extends Notifier<PowerState> {
  ...
  PowerState build() {
    ...
    ref.listen(pduCh1_4Provider, (prev, next) {
      if (next.hasValue) _lastPduCh1_4Update = DateTime.now();
    });
    ref.listen(pduCh5_8Provider, (prev, next) {
    ...
    return PowerState(
      ...
      pduCh1_4: isStale(_lastPduCh1_4Update) ? null : ref.read(pduCh1_4Provider).value,
      pduCh5_8: isStale(_lastPduCh5_8Update) ? null : ref.read(pduCh5_8Provider).value,
    );
  }
}
